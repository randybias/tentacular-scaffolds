...
/** Search for evidence and articles that SUPPORT the original article's thesis */
export default async function run(ctx: Context, input: unknown): Promise<ResearchOutput> {
  const article = input as ArticleContent;

  ctx.log.info(`Researching supporting evidence for: "${article.title}"`);

  const tavily = ctx.dependency("tavily");
  if (!tavily.secret) {
    ctx.log.warn("No tavily API key, returning empty results");
    return { sources: [], searchType: "supporting" };
  }

  // Build a search query focused on supporting evidence
  const thesis = article.content.substring(0, 500);
  const searchQuery = `evidence supporting "${article.title}" data research agrees`;
  ...
  ctx.log.info(`Found ${sources.length} supporting sources`);
  return { sources, searchType: "supporting" };
}
