...
export default async function run(ctx: Context, input: unknown): Promise<ResearchOutput> {
  ...
  ctx.log.info(`Found ${sources.length} contrary sources`);
  return { sources, searchType: "contrary" };
}
