...
/** Analyze contract text for IP assignment, data rights, and confidentiality terms */
export default async function run(ctx: Context, input: unknown): Promise<ReviewOutput> {
  ...
  if (documents.length === 0) {
    ...
    return { reviewType: "ip-rights", findings: [] };
  }

  ctx.log.info(`Reviewing ${documents.length} document(s) for IP rights clauses`);

  const anthropic = ctx.dependency("anthropic");
  if (!anthropic.secret) {
    ctx.log.warn("No anthropic API key, skipping IP rights review");
    return { reviewType: "ip-rights", findings: [] };
  }
  ...
  for (const doc of documents) {
    ...
    const prompt = `You are a legal contract analyst specializing in intellectual property and data rights.
...
- Rights upon termination`;
  ...
  ctx.log.info(`Found ${allFindings.length} IP rights findings`);
  return { reviewType: "ip-rights", findings: allFindings };
}
